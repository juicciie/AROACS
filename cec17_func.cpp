...
void cec17_test_func(double *x, double *f, int nx, int mx,int func_num)
{
	...
	if (ini_flag==0)
	{
		...
		/* Load Matrix M*/
		snprintf(FileName, sizeof(FileName), "input_data/M_%d_D%d.txt", func_num,nx);
		...
		/* Load shift_data */
		snprintf(FileName, sizeof(FileName), "input_data/shift_data_%d.txt", func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
			printf("\n Error: Cannot open input file for reading \n");
		...
		{
			snprintf(FileName, sizeof(FileName), "input_data/shuffle_data_%d_D%d.txt", func_num, nx);
			fpt = fopen(FileName,"r");
			if (fpt==NULL)
			{
				printf("\n Error: Cannot open input file for reading \n");
