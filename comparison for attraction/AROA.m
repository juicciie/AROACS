function [fbest, xbest, Convergence_curve] = AROA(N, maxEvals, lb, ub, dim, fobj)
    ...
    [X, F, evalCounter, best_so_far] = evaluate_population(X, fobj, ub, lb, evalCounter, maxEvals, Inf);
    [fbest, ibest] = min(F);
    xbest = X(ibest,:);
    
    % 记录初始评估的收敛情况
    for i = 1:evalCounter
        Convergence_curve(i) = best_so_far(i);
    end
    % random initialization - Eq (3)
    ...
    for t=1:tmax
        ...
        prev_evalCounter = evalCounter;
        [X, F, evalCounter, best_so_far] = evaluate_population(X, fobj, ub, lb, evalCounter, maxEvals, fbest);
        
        % 记录这次评估的收敛情况
        for i = prev_evalCounter+1:evalCounter
            Convergence_curve(i) = best_so_far(i-prev_evalCounter);
        end
    ...
    % 裁剪只包含有效数据的部分
    Convergence_curve = Convergence_curve(1:evalCounter);
end

function [X, F, evalCounter, best_so_far] = evaluate_population(X, fobj, ub, lb, evalCounter, maxEvals, current_best)
    N = size(X,1);
    F = Inf(N,1);
    X = max(lb, min(ub, X)); % Check space bounds
    
    % 跟踪每次评估的最佳值
    best_so_far = zeros(1, min(N, maxEvals-evalCounter));
    ...
    for i=1:N
        ...
        % 记录每次评估后的最佳值
        best_so_far(evalCounter-(evalCounter-i)) = best_current;
    end
end
